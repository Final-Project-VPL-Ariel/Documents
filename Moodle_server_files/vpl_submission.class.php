...
class mod_vpl_submission {
	...
	$filename = 'test.txt';
        $fg = $this->get_submitted_fgm();
	$myFiles = $othersub->get_submissionfilelistname();
	if (is_writeable($filename))if (is_writable($filename)) {
  ...
  public function grade_reduction(&$reduction, &$percent) {
    ...
    $filename = 'grade_reduction_data.json';
    $data = json_encode([
    'reductionbyevaluation' => $vplinstance->reductionbyevaluation,
    'freeevaluations' => $vplinstance->freeevaluations,
    'nevaluations' => $this->instance->nevaluations,
]);
