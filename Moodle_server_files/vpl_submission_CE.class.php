...
class mod_vpl_submission_CE extends mod_vpl_submission {
    ...
    public function prepare_execution($type, &$already = array(), $vpl = null) {
        ...
        if ($user = $DB->get_record( 'user', array ( 'id' => $subinstance->userid ) )) {
            ...
            $info .= vpl_bash_export( 'MOODLE_USER_REAL_ID', $user->idnumber );
        }

            // Append the duedate and startdate to the $info string
            $info .= vpl_bash_export('VPL_DUEDATE', $vpl->get_instance()->duedate);
            $info .= vpl_bash_export('VPL_STARTDATE', $vpl->get_instance()->startdate);

            $user_id = $subinstance->userid;
            $vpl_id = $vpl->get_instance()->id;
            ...
            o.duedate,
            o.freeevaluations,
            o.reductionbyevaluation,
            ao.userid,
            ao.groupid
        FROM
            mdl_vpl_overrides o
        LEFT JOIN
            mdl_vpl_assigned_overrides ao ON o.vpl = ao.vpl AND o.id = ao.override
        WHERE
            ao.userid = :user_id and o.vpl = :vpl_id;
            ";
            ...
            if ($result) {
                $info .= vpl_bash_export('VPL_REDUCTION_POINTS', $result->reductionbyevaluation);
                $info .= vpl_bash_export('VPL_DUEDATE_OVERRIDE', $result->duedate);
                $info .= vpl_bash_export('VPL_SUBMISSION_DATE' , $subinstance->datesubmitted);
                $info .= vpl_bash_export('VPL_FREE_EVALUATIONS', $result->freeevaluations);
            }
